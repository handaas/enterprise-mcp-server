...
@mcp.tool()
def enterprise_get_keyword_search(matchKeyword: str, pageIndex: int = None, pageSize: int = 50) -> dict:
    """
    关键词模糊查询企业
...
def enterprise_get_enterprise_base_info(keyword: str) -> dict:
    ...
    参数:
      - keyword: 企业全称 类型：string
    返回值:
      - base_info: 企业工商信息 类型：list of dict:
      - desc: 企业简介 类型：string
      - business_info: 企业业务 类型：list of dict:
      - tag: 企业标签 类型：list of dict:
    """
    try:
        # 调用模糊搜索接口
...
def enterprise_get_enterprise_holder_info(keyword: str) -> dict:
    ...
    参数:
      - keyword: 企业全称 类型：string
    返回值:
      - holderList: 股东列表（工商公示） 类型：list of dict:
        - holderType: 股东类型 类型：string
        - name: 股东名称 类型：string
        - humanId: 人员id 类型：string
        - payAmount: 实缴金额 类型：list of dict:
        - ratio: 持股比例 类型：float
        - subscriptionDetail: 认缴信息 类型：list of dict:
      - stockHolderList：股东列表（最新公示）来自于上市信息 类型：list of dict:
        - entityType: 主体类型 类型：string
        - nameId: 企业id 类型：string
        - name: 企业名称 类型：string
        - shareHolding: 持有金额 类型：int
        - shareHoldingRatio: 持股比例 类型：float
        - shareHoldingType: 持股类型 类型：string
    """
    try:
        # 调用模糊搜索接口
...
def enterprise_get_enterprise_invest_info(keyword: str) -> dict:
    ...
    返回值:
      - resultList: 对外投资结果列表 类型：list of dict:
        - addressValue: 被投资公司所属地区 类型：list of dict:
        - business: 被投资公司经营范围 类型：string
        - foundTime: 被投资公司成立日期 类型：string
        - isListed: 被投资公司上市状态 类型：int
        - legalRepresentative: 被投资公司法定代表人 类型：string
        - name: 对外投资企业名 类型：string
        - operStatus: 对外投资企业经营状态 类型：string
        - ratio: 占股比例 类型：float
        - regCapital: 被投资公司注册资本 类型：list of dict:
        - scCode: 对外投资企业统一信用编码 类型：string
        - subscriptionAmount: 投资金额信息 类型：list of dict:
    """
    try:
        # 调用模糊搜索接口
...
def enterprise_get_enterprise_branch_info(keyword: str) -> dict:
    ...
    返回值:
      - resultList: 对外投资结果列表  类型：list of dict:
        - addressValue: 地址信息 类型：list of dict:
        - foundTime: 成立时间 类型：string
        - legalRepresentative: 法定代表人 类型：string
        - name: 机构名称 类型：string
        - operStatus: 经营状态 类型：list of dict:
        - orgCode: 组织机构代码 类型：string
        - registrationAuthority: 登记机关 类型：string
        - socialCreditCode: 统一社会信用代码 类型：string
      - total: 总数 类型：int
    """
    try:
        # 调用模糊搜索接口
...
def enterprise_get_enterprise_main_person_info(keyword: str) -> dict:
    ...
    返回值:
      - resultList: 对外投资结果列表 类型：list of dict:
        - name: 成员名称 类型：string
        - position: 职位 类型：list of dict:
        - ratio: 持股比例 类型：float
        - relatedEnterpriseCurrentNum: 现任职企业数 类型：int
        - relatedEnterpriseHistoryNum: 曾任职企业数 类型：int
      - total: 总数 类型：int
    """
    try:
        # 调用模糊搜索接口
